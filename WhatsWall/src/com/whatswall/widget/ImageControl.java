...
public class ImageControl extends ImageView {
	...
	float bigScale = 1.01f; // 默认放大倍数
	float smallScale = 0.99f; // 默认放大倍数
	Boolean isBig = false; // 是否是放大状态
	boolean isGoonBig = false; // 是否继续放大
	 ...
	 *            点击点Y坐标
	 */
	private void changeSize(float x, float y , float dif) {
		if (dif<0) {
			// 如果处于最大状态，则还原
			imgMatrix.reset();
			imgMatrix.postScale(smallScale, smallScale);
			imgMatrix.postTranslate(subX, subY);
			isBig = false;
		} else {
